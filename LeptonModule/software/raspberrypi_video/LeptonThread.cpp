...
LeptonThread::LeptonThread() : QThread()
{
	...
	//
	typeColormap = 1; // 1:colormap_rainbow  /  2:colormap_grayscale  /  3:colormap_ironblack(default)
	...
	//
	spiSpeed = 30 * 1000 * 1000; // SPI bus speed 20MHz
...
{
	ros::NodeHandle nh;
	ros::Publisher imagePub = nh.advertise<sensor_msgs::Image>("/image", 1);
	...
	while(true) {
		...
		// Convert QImage to cv::Mat for use with cv_bridge
		cv::Mat cvImage(myImage.height(), myImage.width(), CV_8UC3, myImage.bits());
		cv::cvtColor(cvImage, cvImage, cv::COLOR_RGB2BGR); // QImage stores in RGB, but cv_bridge expects BGR

		// Convert cv::Mat to sensor_msgs::Image using cv_bridge
		cv_bridge::CvImage cvImageMsg;
		cvImageMsg.encoding = "rgb8"; // BGR format
		cvImageMsg.image = cvImage;

		// Publish the sensor_msgs::Image message
		imagePub.publish(cvImageMsg.toImageMsg());
